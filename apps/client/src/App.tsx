...
const uploadVid=async(f)=>{try{const fd=new FormData();fd.append("file",f);const r=await fetch(WORKER+"/api/uploads",{method:"POST",headers:{Authorization:"Bearer "+(sessionStorage.getItem("fc_token")||"")},body:fd});const j=await r.json();return j.ok?j.data.url:URL.createObjectURL(f);}catch{return URL.createObjectURL(f);}};
const hu=async(e)=>{const fs=Array.from(e.target.files||[]);if(!fs.length)return;const np:any[]=await Promise.all((fs as File[]).map(async(f:File)=>{const isVid=f.type.startsWith("video");if(isVid)return {data:await uploadVid(f),name:f.name,date:new Date().toISOString().split("T")[0],type:"video"};return {data:"pending",blob:f,name:f.name,date:new Date().toISOString().split("T")[0],type:"image"};}));const imgs=np.filter(x=>x.type==="image");const vids=np.filter(x=>x.type==="video");if(imgs.length===0){onPhotos([...photos,...vids]);e.target.value="";return;}await Promise.all(imgs.map(item=>new Promise(r=>{const rd=new FileReader();rd.onload=()=>{item.data=rd.result;r(item);};rd.readAsDataURL(item.blob);})));onPhotos([...photos,...imgs,...vids]);e.target.value="";};
...
<label style={{...S.btn,fontSize:12,padding:"4px 10px",background:"#1E3A5F",cursor:"pointer"}}><input type="file" multiple style={{display:"none"}} onChange={async e=>{const files=Array.from(e.target.files||[]);if(!files.length)return;const np:any[]=await Promise.all((files as File[]).map(async(f:File)=>{const isVid=f.type.startsWith("video");if(isVid)return {data:await uploadVid(f),name:f.name,type:"video"};return {data:"pending",blob:f,name:f.name,type:"image"};}));const imgs=np.filter(x=>x.type==="image");const vids=np.filter(x=>x.type==="video");if(imgs.length===0){onUpdVendor(vn.id,{vendorPhotos:[...(vn.vendorPhotos||[]),...vids]});e.target.value="";return;}await Promise.all(imgs.map(item=>new Promise(r=>{const rd=new FileReader();rd.onload=()=>{item.data=rd.result;r(item);};rd.readAsDataURL(item.blob);})));onUpdVendor(vn.id,{vendorPhotos:[...(vn.vendorPhotos||[]),...imgs,...vids]});e.target.value="";}}/> 📷 Upload Photo/Video</label></div>
