...
{(()=>{
  ...
  return <button style={{...S.btn,width:"100%",background:"#92400E",color:"#FDE68A",padding:"10px 12px",marginTop:8,fontWeight:700,border:"2px solid #F59E0B",opacity:allReady?1:0.4}} disabled={!allReady}
    onClick={()=>{
...
{(vn.declinedFindings||[]).length>0&&<div style={{marginTop:6}}>
  {(vn.declinedFindings||[]).map((df: any,dfi: any)=><div key={dfi} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"5px 8px",marginBottom:3,borderRadius:5,background:"#1A0808",border:"1px solid #7F1D1D",opacity:0.7}}>
    <span style={{fontSize:13,color:"#FCA5A5",textDecoration:"line-through",flex:1}}>🔍 {df.desc}</span>
    <div style={{display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
      <span style={{fontSize:13,color:"#6B7280",textDecoration:"line-through"}}>${df.price||0}</span>
      <span style={{fontSize:11,fontWeight:700,color:"#FCA5A5",background:"#7F1D1D",padding:"2px 6px",borderRadius:4}}>❌ DECLINED</span>
    </div>
  </div>)}
</div>}
...
{(vn.vendorFindings||[]).some((x: any)=>x.approved)&&(()=>{const appTotal=(vn.vendorFindings||[]).filter((x: any)=>x.approved).reduce((s: any,x: any)=>s+(Number(x.price)||0),0);return<div style={{marginTop:8,padding:"8px 12px",borderRadius:8,background:"#1A1200",border:"1px solid #92400E",display:"flex",flexDirection:"column",gap:2}}><div style={{fontSize:13,color:"#F97316",fontWeight:700}}>+ ${appTotal.toLocaleString()} approved findings</div><div style={{fontSize:17,fontWeight:800,color:"#FBBF24",borderTop:"1px solid #92400E",paddingTop:4,marginTop:2}}>Total: ${(lt+appTotal).toLocaleString()}</div></div>;})()}
