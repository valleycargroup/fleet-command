...
{(()=>{const vfList=vn.vendorFindings||[];const allReady=vfList.length>0&&vfList.every((x: any)=>x.desc&&x.price>0);return vfList.length>0&&!vn.findingsSubmitted?<button style={{...S.btn,width:"100%",background:"#92400E",color:"#FDE68A",padding:10,fontSize:14,marginTop:8,border:"2px solid #F59E0B",opacity:allReady?1:0.4}} disabled={!allReady} onClick={()=>{if(!allReady){notify&&notify("⚠️ All findings need description and price");return;}onUpdVendor(vn.id,{findingsSubmitted:true,findingsSubmittedDate:new Date().toISOString().split("T")[0]});if(typeof fireEmail==="function"&&vehicle){const origItems=(vn.lineItems||[]).filter((x: any)=>x.accepted);const origTotal=origItems.reduce((s: any,x: any)=>s+(Number(x.price)||0),0);const findItems=vn.vendorFindings||[];const findTotal=findItems.reduce((s: any,x: any)=>s+(Number(x.price)||0),0);fireEmail("buyer_bid_submitted",{buyer:vehicle.buyingBroker||"Buyer",vendor:{name:vn.name},vehicle:vData(vehicle),category:cat.label+" — FINDINGS",categoryKey:cat.key,lineItems:[...origItems.map((x: any)=>({desc:x.desc,price:x.price,costType:x.costType||"ws"})),...findItems.map((x: any)=>({desc:"🔍 "+x.desc,price:x.price,costType:"ws"}))],totalBid:origTotal+findTotal});}setExp(false);}}>🔍 Submit Findings</button>:null;})()}
