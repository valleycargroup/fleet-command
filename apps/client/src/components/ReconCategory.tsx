...
const autoExpand = deepLinkCat === cat.key;
const onClearDeepLink = () => setDeepLinkCat(null);
const [ei,setEi]=useState("");const [no,setNo]=useState(false);const [nv,setNv]=useState(task?.notes||"");const [po,setPo]=useState(false);const [exp,setExp]=useState(autoExpand||false);const fr=useRef(null as any);
...
<label style={{fontSize:12,color:"#6B7280",display:"flex",alignItems:"center",gap:4}} onClick={(e: any)=>e.stopPropagation()}>
...
{need&&sv&&<div style={{marginTop:4}}><span style={{fontSize:14,fontWeight:700,color:"#FFF"}}>{sv.name}</span><span style={{color:"#FBBF24",marginLeft:8}}>${(()=>{const acceptedTotal=(sv.lineItems||[]).filter((x: any)=>x.accepted&&!x.declined).reduce((s: any,x: any)=>s+(Number(x.price)||0),0);if(acceptedTotal>0)return acceptedTotal.toLocaleString();const bidTotal=(sv.lineItems||[]).filter((x: any)=>!x.declined).reduce((s: any,x: any)=>s+(Number(x.price)||0),0);return bidTotal.toLocaleString()+(sv.bidLocked&&task?.status==="estimated"?" pending":"");})()}</span></div>}
...
{need&&(task.completedRounds||[]).length>0&&<div style={{marginTop:2}}>{(task.completedRounds||[]).map((rd: any,ri: any)=><div key={ri} style={{fontSize:10,color:"#34D399"}}>✅ R{ri+1}: {rd.tasks||"completed"} — ${rd.cost||0}{rd.vendor?" — "+rd.vendor:""}</div>)}</div>}
{need&&(()=>{const vs=task.vendors||[];const dec=vs.filter((v2: any)=>v2.declined).length;return dec>0?<div style={{fontSize:12,color:"#FCA5A5",marginTop:2}}>❌ {dec}/{vs.length} vendors declined</div>:null;})()}
...
<input style={{...S.fi,fontSize:16,padding:"10px 14px",flex:1}} type="date" value={task.oemDropDate||""} onChange={(e: any)=>onUpdateTask({oemDropDate:e.target.value,status:e.target.value?"assigned":"unassigned",vendorName:task.oemDealer||"OEM Dealer",dateAssigned:e.target.value||task.dateAssigned})}/>
</div></label>
{task.oemDropDate&&<div style={{fontSize:14,color:"#FBBF24",fontWeight:600,marginTop:4}}>📅 Dropped Off: {fmtDate(task.oemDropDate)}</div>}
<label style={{...S.fl,fontSize:14,marginTop:8}}>Problems / Concerns *<textarea style={{...S.fi,fontSize:14,minHeight:80,resize:"vertical"}} value={task.oemProblems||""} onChange={(e: any)=>onUpdateTask({oemProblems:e.target.value})} placeholder="Describe what needs to be looked at..."/></label>
...
<label style={{...S.fl,fontSize:13,marginTop:6}}>Warranty Notes<textarea style={{...S.fi,fontSize:13,minHeight:50,resize:"vertical",width:"100%",boxSizing:"border-box"}} value={task.oemWarrantyNotes||""} onChange={(e: any)=>onUpdateTask({oemWarrantyNotes:e.target.value})} placeholder="Recall #, warranty type, what's covered..."/></label>
</div>}
...
{(task.oemItems||[]).map((item: any,ii: any)=><div key={item.id} style={{padding:8,marginBottom:4,borderRadius:6,background:"#0D0D1A",border:"1px solid #2A2A3E"}}>
<div style={{display:"flex",gap:6,alignItems:"center",marginBottom:4}}>
<input style={{...S.fi,fontSize:14,padding:"6px 10px",flex:1}} placeholder="Work description" value={item.desc} onChange={(e: any)=>{const items=[...(task.oemItems||[])];items[ii]={...items[ii],desc:e.target.value};onUpdateTask({oemItems:items});}}/>
<input style={{...S.fi,fontSize:14,padding:"6px 10px",width:100,textAlign:"right"}} type="number" placeholder="$" value={item.price||""} onChange={(e: any)=>{const items=[...(task.oemItems||[])];items[ii]={...items[ii],price:Number(e.target.value)};onUpdateTask({oemItems:items});}}/>
<button style={{background:"none",border:"none",color:"#F87171",fontSize:16,cursor:"pointer"}} onClick={()=>{const items=(task.oemItems||[]).filter((_: any,j: any)=>j!==ii);onUpdateTask({oemItems:items});}}>✕</button>
</div>
...
<div style={{fontSize:16,fontWeight:800,color:"#E5E7EB"}}>Total: <span style={{color:"#FBBF24",fontSize:20}}>${(task.oemItems||[]).reduce((s: any,x: any)=>s+(x.accepted?Number(x.price)||0:0),0).toLocaleString()}</span> <span style={{fontSize:12,color:"#6B7280"}}>(accepted)</span></div>
{(()=>{const items=task.oemItems||[];const ws=items.filter((x: any)=>x.accepted&&x.costType!=="retail").reduce((s: any,x: any)=>s+(Number(x.price)||0),0);const ret=items.filter((x: any)=>x.accepted&&x.costType==="retail").reduce((s: any,x: any)=>s+(Number(x.price)||0),0);return (ws>0||ret>0)?<div style={{display:"flex",gap:12,marginTop:4}}>{ws>0&&<span style={{fontSize:13,color:"#93C5FD",fontWeight:600}}>W/S: ${ws.toLocaleString()}</span>}{ret>0&&<span style={{fontSize:13,color:"#67E8F9",fontWeight:600}}>🏪 Retail: ${ret.toLocaleString()}</span>}</div>:null;})()}
</div>}
</div>}
</div>}
{task.oemDropDate&&((task.oemWarranty)||(task.oemItems||[]).some((x: any)=>x.accepted))&&!task.oemWorkStarted&&<button style={{...S.btn,width:"100%",background:"#78590A",color:"#FDE68A",padding:12,fontSize:16,marginTop:8}} onClick={(e: any)=>{e.target.disabled=true;onUpdateTask({oemWorkStarted:true,oemWorkStartedDate:new Date().toISOString().split("T")[0],status:"started",dateStarted:new Date().toISOString().split("T")[0]});
if(typeof fireEmail==="function")fireEmail("vendor_work_started",{buyer:vehicle?.buyingBroker||"Buyer",vendor:{name:task.oemDealer||"OEM Dealer"},vehicle:vData(vehicle),category:"OEM Dealer — "+cat.label,lineItems:(task.oemItems||[]).filter((x: any)=>x.accepted).map((x: any)=>({desc:x.desc,price:x.price,costType:x.costType||"ws"})),totalApproved:(task.oemItems||[]).filter((x: any)=>x.accepted).reduce((s: any,x: any)=>s+(Number(x.price)||0),0),startedDate:new Date().toISOString().split("T")[0]});}}>🔧 Work Started</button>}
...
<button style={{...S.btn,width:"100%",background:"#166534",color:"#6EE7B7",padding:12,fontSize:16,marginTop:8}} onClick={(e: any)=>{e.target.disabled=true;onUpdateTask({oemPickedUp:true,oemPickedUpDate:new Date().toISOString().split("T")[0],status:"complete",dateCompleted:new Date().toISOString().split("T")[0]});if(typeof onCmp==="function")onCmp();
if(typeof fireEmail==="function")fireEmail("buyer_work_complete",{buyer:vehicle?.buyingBroker||"Buyer",vendor:{name:task.oemDealer||"OEM Dealer"},vehicle:vData(vehicle),category:"OEM Dealer — "+(task.oemDealer||""),lineItems:[...(task.oemItems||[]).filter((x: any)=>x.accepted).map((x: any)=>({desc:x.desc,price:x.price,costType:x.costType||"ws"})),...(task.oemWarranty?[{desc:"🛡️ WARRANTY: "+(task.oemWarrantyNotes||"Warranty covered"),price:0,costType:"ws"}]:[])],totalCost:(task.oemItems||[]).filter((x: any)=>x.accepted).reduce((s: any,x: any)=>s+(Number(x.price)||0),0)});}}>✅ Picked Up — Work Complete</button>
</div>}
...
{!isVendor&&!vn.selected&&task.status!=="complete"&&task.status!=="started"&&<button style={{padding:"6px 14px",borderRadius:6,background:"transparent",color:"#6B7280",fontSize:12,fontWeight:700,border:"1px solid #4B5563",cursor:"pointer",marginLeft:6}} onClick={()=>{showConfirm(`Remove ${vn.name} from this recon task? This cannot be undone.`,()=>{const others=(task.vendors||[]).filter((v2: any)=>v2.id!==vn.id);const newStatus=others.length===0?"unassigned":task.status;onUpdateTask({vendors:others,status:newStatus});});}} >🗑 Remove</button>}
</div>
...
:<input style={{...S.fi,fontSize:12,padding:"3px 6px",width:110}} type="date" onChange={(e: any)=>{const items=[...(vn.lineItems||[])];const ix=items.findIndex((x: any)=>x.id===w.id);if(ix>=0)items[ix]={...items[ix],partOrdered:true,partOrderedDate:e.target.value};onUpdVendor(vn.id,{lineItems:items});}}/>}
</div>
<div style={{display:"flex",alignItems:"center",gap:4}}>
...
:li.partOrdered?<input style={{...S.fi,fontSize:12,padding:"3px 6px",width:110}} type="date" onChange={(e: any)=>{const items=[...(vn.lineItems||[])];const ix=items.findIndex((x: any)=>x.id===w.id);if(ix>=0)items[ix]={...items[ix],partArrived:true,partArrivedDate:e.target.value};onUpdVendor(vn.id,{lineItems:items});}}/>
...
:li.partArrived?<input style={{...S.fi,fontSize:12,padding:"3px 6px",width:110}} type="date" onChange={(e: any)=>{const items=[...(vn.lineItems||[])];const ix=items.findIndex((x: any)=>x.id===w.id);if(ix>=0)items[ix]={...items[ix],partInstalled:true,partInstalledDate:e.target.value};onUpdVendor(vn.id,{lineItems:items});}}/>
:<span style={{fontSize:12,color:"#4B5563"}}>--</span>}
</div>
</div>}
</div>}
...
{(()=>{const items=vn.lineItems||[];const wsTotal=items.filter((x: any)=>x.accepted&&x.costType!=="retail").reduce((s: any,x: any)=>s+(Number(x.price)||0),0);const retailTotal=items.filter((x: any)=>x.accepted&&x.costType==="retail").reduce((s: any,x: any)=>s+(Number(x.price)||0),0);return (wsTotal>0||retailTotal>0)?<div style={{display:"flex",gap:12,marginTop:6}}>
...
{isVendor?<label style={{...S.fl,fontSize:15,fontWeight:700}}>📅 ETA Completion Date *<DateIn style={{fontSize:16,padding:"10px 12px",marginTop:4}} value={vn.etaDone||""} onChange={(v2: any)=>onUpdVendor(vn.id,{etaDone:v2})}/></label>:vn.etaDone?<div style={{...S.fl,fontSize:14}}><span style={{color:"#9CA3AF"}}>📅 Vendor ETA:</span> <b style={{color:"#60A5FA"}}>{fmtDate(vn.etaDone)}</b></div>:<div style={{...S.fl,fontSize:13,color:"#6B7280",fontStyle:"italic"}}>📅 Vendor will set completion date when bidding</div>}
</div>}
{((wt.filter((w: any)=>w.isPart).length>0)||(cat.key==="parts"&&wt.length>0))&&vn.selected&&(()=>{const pts=(cat.key==="parts"?wt:wt.filter((w: any)=>w.isPart)).filter((w: any)=>{const li9=(vn.lineItems||[]).find((x: any)=>x.id===w.id)||{};return !li9.declined&&li9.accepted;});if(pts.length===0)return null;const arrived=pts.filter((w: any)=>{const li=(vn.lineItems||[]).find((x: any)=>x.id===w.id)||{};return li.partArrived;}).length;const allIn=pts.length>0&&arrived===pts.length;return <div style={{padding:12,background:"#1A1A2E",borderRadius:8,marginBottom:8}}>
<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
...
{pts.map((w: any)=>{const li=(vn.lineItems||[]).find((x: any)=>x.id===w.id)||{};return <div key={w.id} style={{padding:10,marginBottom:6,borderRadius:6,background:"#0D0D1A",border:`1px solid ${li.partInstalled?"#166534":li.partArrived?"#3B82F6":"#2A2A3E"}`}}>
...
<label style={{...S.fl,fontSize:13,flex:1}}>Part Cost $<input style={{...S.fi,fontSize:14,padding:"6px 10px"}} type="number" placeholder="$" value={li.partPrice||li.price||""} onChange={(e: any)=>{const items=[...(vn.lineItems||[])];const ix=items.findIndex((x: any)=>x.id===w.id);if(ix>=0)items[ix]={...items[ix],partPrice:Number(e.target.value)};onUpdVendor(vn.id,{lineItems:items});}}/></label>
{li.partPrice>0&&!li.partApproved&&<button style={{...S.btn,fontSize:12,background:"#1E3A5F",padding:"6px 12px",whiteSpace:"nowrap"}} onClick={()=>{const items=[...(vn.lineItems||[])];const ix=items.findIndex((x: any)=>x.id===w.id);if(ix>=0)items[ix]={...items[ix],partApproved:true,partApprovedDate:new Date().toISOString().split("T")[0]};onUpdVendor(vn.id,{lineItems:items});}}>Approve</button>}
...
<label style={{fontSize:12,color:li.partOrdered?"#93C5FD":"#6B7280",fontWeight:600,cursor:"pointer"}}><input type="checkbox" checked={li.partOrdered||false} onChange={(e: any)=>{const items=[...(vn.lineItems||[])];const ix=items.findIndex((x: any)=>x.id===w.id);if(ix>=0)items[ix]={...items[ix],partOrdered:e.target.checked,partOrderedDate:e.target.checked?new Date().toISOString().split("T")[0]:""};onUpdVendor(vn.id,{lineItems:items});}}/> Ordered</label>
...
<label style={{fontSize:12,color:li.partArrived?"#34D399":"#6B7280",fontWeight:600,cursor:"pointer"}}><input type="checkbox" checked={li.partArrived||false} onChange={(e: any)=>{const items=[...(vn.lineItems||[])];const ix=items.findIndex((x: any)=>x.id===w.id);const checked=e.target.checked;const today=new Date().toISOString().split("T")[0];if(ix>=0)items[ix]={...items[ix],partArrived:checked,partArrivedDate:checked?today:"",partStatus:checked?"received":items[ix].partStatus};onUpdVendor(vn.id,{lineItems:items});}}/> Arrived</label>
...
:li.partArrived?<button style={{fontSize:12,padding:"4px 8px",background:"#16A34A",color:"#FFF",border:"none",borderRadius:4,cursor:"pointer",width:"100%",fontWeight:700}} onClick={()=>{const items=[...(vn.lineItems||[])];const ix=items.findIndex((x: any)=>x.id===w.id);if(ix>=0)items[ix]={...items[ix],partInstalled:true,partInstalledDate:new Date().toISOString().split("T")[0]};onUpdVendor(vn.id,{lineItems:items});}}>🔧 Install</button>
:<span style={{fontSize:11,color:"#4B5563"}}>—</span>}
</div>
</div>
</div>;})}</div>;})()}
...
<input style={{...S.fi,fontSize:12,flex:1}} placeholder="Work found" value={vf.desc} onChange={(e: any)=>{const f=[...(vn.vendorFindings||[])];f[vfi]={...f[vfi],desc:e.target.value};onUpdVendor(vn.id,{vendorFindings:f});}}/>
...
<label style={{...S.btn,fontSize:12,padding:"4px 10px",background:"#1E3A5F",cursor:"pointer"}}><input type="file" multiple style={{display:"none"}} onChange={async (e: any)=>{const files=Array.from(e.target.files||[]);if(!files.length)return;const np:any[]=await Promise.all((files as File[]).map(async(f:File)=>{const isVid=f.type.startsWith("video");if(isVid)return {data:await uploadVid(f),name:f.name,type:"video"};return {data:"pending",blob:f,name:f.name,type:"image"};}));const imgs=np.filter(x=>x.type==="image");const vids=np.filter(x=>x.type==="video");if(imgs.length===0){onUpdVendor(vn.id,{vendorPhotos:[...(vn.vendorPhotos||[]),...vids]});e.target.value="";return;}await Promise.all(imgs.map((item: any)=>new Promise(r=>{const rd=new FileReader();rd.onload=()=>{item.data=rd.result;r(item);};rd.readAsDataURL(item.blob);})));onUpdVendor(vn.id,{vendorPhotos:[...(vn.vendorPhotos||[]),...imgs,...vids]});e.target.value="";}}/>📷 Upload Photo/Video</label></div>
{(vn.vendorPhotos||[]).length>0&&<div style={{display:"flex",gap:4,flexWrap:"wrap"}}>{(vn.vendorPhotos||[]).map((p: any,pi: any)=><div key={pi} style={{width:50,height:50,borderRadius:4,overflow:"hidden",border:"1px solid #2A2A3E",cursor:"pointer",position:"relative"}} onClick={()=>setLbImg(p)}>{p.type==="video"?<div style={{width:"100%",height:"100%",background:"#0D0D1A",display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:18}}>🎬</span></div>:<img src={p.data} style={{width:"100%",height:"100%",objectFit:"cover"}}/>}</div>)}</div>}</div>
{!vn.bidLocked&&ap&&vn.etaDone&&<div style={{background:"#052E16",borderRadius:8,overflow:"hidden"}}>
<div style={{padding:"8px 12px"}}>{wt.map((w: any)=>{const li2=(vn.lineItems||[]).find((x: any)=>x.id===w.id);return <div key={w.id} style={{display:"flex",justifyContent:"space-between",padding:"3px 0",fontSize:13,color:"#6EE7B7"}}><span>{w.desc}</span><span style={{fontWeight:700}}>${Number(li2?.price)||0}</span></div>;})}</div>
<button style={{...S.btn,width:"100%",background:"#166534",color:"#6EE7B7",borderRadius:0,fontSize:16,padding:12}} onClick={()=>{onUpdVendor(vn.id,{bidLocked:true,estimate:bidTotal},{status:"estimated",estimate:bidTotal});
if(typeof fireEmail==="function"&&vehicle){const items=(vn.lineItems||[]).map((x: any)=>({desc:x.desc,price:x.price,costType:x.costType||"ws",isPart:x.isPart}));fireEmail("buyer_bid_submitted",{buyer:vehicle.buyingBroker||"Buyer",vendor:{name:vn.name},vehicle:vData(vehicle),category:cat.label,categoryKey:cat.key,lineItems:items,totalBid:bidTotal});}setExp(false);}}>Submit Bid — ${bidTotal.toLocaleString()}</button>
</div>}
...
{(()=>{const vfList=vn.vendorFindings||[];const allReady=vfList.length>0&&vfList.every((x: any)=>x.desc&&x.price>0);const findTotal=vfList.reduce((s: any,x: any)=>s+(Number(x.price)||0),0);return vfList.length>0&&!vn.findingsSubmitted?<button style={{...S.btn,width:"100%",background:"#92400E",color:"#FDE68A",padding:10,fontSize:14,marginTop:8,fontWeight:700,border:"2px solid #F59E0B",opacity:allReady?1:0.4}} disabled={!allReady} onClick={()=>{if(!allReady){notify&&notify("⚠️ All findings need description and price");return;}onUpdVendor(vn.id,{findingsSubmitted:true,findingsSubmittedDate:new Date().toISOString().split("T")[0]});if(typeof fireEmail==="function"&&vehicle){const origItems=(vn.lineItems||[]).filter((x: any)=>x.accepted);const origTotal=origItems.reduce((s: any,x: any)=>s+(Number(x.price)||0),0);fireEmail("buyer_bid_submitted",{buyer:vehicle.buyingBroker||"Buyer",vendor:{name:vn.name},vehicle:vData(vehicle),category:cat.label+" — FINDINGS",categoryKey:cat.key,lineItems:[...origItems.map((x: any)=>({desc:x.desc,price:x.price,costType:x.costType||"ws"})),...vfList.map((x: any)=>({desc:"🔍 "+x.desc,price:x.price,costType:"ws"}))],totalBid:origTotal+findTotal});}notify&&notify(`🔍 Findings submitted — $${findTotal.toLocaleString()} pending review`);setExp(false);}}>🔍 Submit {vfList.length} Finding{vfList.length!==1?"s":""} (+${findTotal.toLocaleString()}) · Total ${(lt+(vn.bidAdjustment||0)+findTotal).toLocaleString()}</button>:null;})()}
...
{(vn.vendorFindings||[]).length>0&&!vn.findingsSubmitted&&!(vn.vendorFindings||[]).every((x: any)=>x.desc&&x.price>0)&&<div style={{fontSize:12,color:"#F87171",marginTop:4}}>⚠️ Each finding needs: description + price</div>}
{(vn.vendorFindings||[]).some((x: any)=>x.approved)&&<div style={{marginTop:4,fontSize:14,color:"#F97316",fontWeight:700}}>+ Findings: ${(vn.vendorFindings||[]).filter((x: any)=>x.approved).reduce((s: any,x: any)=>s+(Number(x.price)||0),0).toLocaleString()}</div>}
{(vn.vendorFindings||[]).some((x: any)=>x.approved)&&<div style={{fontSize:16,fontWeight:800,color:"#FBBF24",marginTop:2}}>Total: ${(lt+(vn.vendorFindings||[]).filter((x: any)=>x.approved).reduce((s: any,x: any)=>s+(Number(x.price)||0),0)).toLocaleString()}</div>}
{(()=>{const vf=vn.vendorFindings||[];if(vf.length===0||!vn.findingsSubmitted)return null;const allDecided=vf.every((x: any)=>x.approved||x.declined);const alreadySent=vn.findingsDecisionSent;if(!allDecided)return <div style={{fontSize:12,color:"#F59E0B",marginTop:6}}>⚠️ Accept or decline all findings before sending decision</div>;if(alreadySent)return <div style={{fontSize:12,color:"#34D399",marginTop:6}}>✅ Decision sent to vendor {fmtDate(vn.findingsDecisionSentDate)}</div>;return <button style={{...S.btn,width:"100%",background:"#1E3A5F",color:"#93C5FD",padding:12,fontSize:15,marginTop:8,border:"2px solid #3B82F6",fontWeight:800}} onClick={(e: any)=>{e.target.disabled=true;const accF=vf.filter((x: any)=>x.approved);const decF=vf.filter((x: any)=>x.declined);const accTotal=accF.reduce((s: any,x: any)=>s+(Number(x.price)||0),0);const origItems=(vn.lineItems||[]).filter((x: any)=>x.accepted&&!x.declined);const origTotal=origItems.reduce((s: any,x: any)=>s+(Number(x.price)||0),0);
if(typeof fireEmail==="function"&&vehicle){fireEmail("vendor_bid_accepted",{vendor:{name:vn.name},vehicle:vData(vehicle),lineItems:[...origItems.map((x: any)=>({desc:"✅ "+x.desc,price:x.price,costType:x.costType||"ws"})),...accF.map((x: any)=>({desc:"✅ 🔍 "+x.desc,price:x.price,costType:x.findingCostType||"ws"})),...decF.map((x: any)=>({desc:"❌ "+x.desc+" (DECLINED)",price:0,costType:x.findingCostType||"ws"}))],totalApproved:origTotal+accTotal,approvedBy:vehicle.buyingBroker||"Buyer",approvedDate:new Date().toISOString().split("T")[0]});}
const newTasks=accF.map((f: any,fi: any)=>({id:"wf"+Date.now()+"_"+fi,desc:"🔍 "+f.desc,isPart:false,isFromFinding:true,findingPrice:Number(f.price)||0,findingCostType:f.findingCostType||"ws"}));if(newTasks.length>0){const curWt=[...(task.workTasks||[]),...newTasks];const updatedVn={...vn,findingsDecisionSent:true,findingsDecisionSentDate:new Date().toISOString().split("T")[0],lineItems:[...(vn.lineItems||[]),...newTasks.map((nt: any)=>({id:nt.id,desc:nt.desc,price:nt.findingPrice,accepted:true,costType:nt.findingCostType,isPart:false}))],vendorFindings:[],findingsSubmitted:false};const updatedVendors=(task.vendors||[]).map((v2: any)=>v2.id===vn.id?updatedVn:v2);const taskExtra: any={workTasks:curWt,vendors:updatedVendors};if(task.status==="complete"){const prevTasks=(task.workTasks||[]).map((w: any)=>w.desc).join(", ");const prevCost=origTotal;taskExtra.status="started";taskExtra.dateStarted=new Date().toISOString().split("T")[0];taskExtra.dateCompleted=null;taskExtra.roundPhotoStart=(task.photos||[]).length;taskExtra.completedRounds=[...(task.completedRounds||[]),{tasks:prevTasks,cost:prevCost,date:task.dateCompleted||new Date().toISOString().split("T")[0],vendor:vn.name}];}else if(task.status==="started"){taskExtra.roundPhotoStart=(task.photos||[]).length;}onUpdateTask(taskExtra);}else{onUpdVendor(vn.id,{findingsDecisionSent:true,findingsDecisionSentDate:new Date().toISOString().split("T")[0]});}notify&&notify("📩 Decision sent"+(newTasks.length>0?" — "+newTasks.length+" findings added as tasks":""));}}>📩 Send Findings Decision to Vendor</button>;})()}
</div>}
{!vn.bidLocked&&ap&&!vn.etaDone&&<div style={{fontSize:12,color:"#F87171",padding:6}}>⚠️ Set ETA completion date to submit bid</div>}
{!vn.declined&&!vn.selected&&task.status!=="complete"&&<div style={{marginTop:10,padding:2,background:"#3B1515",borderRadius:8,border:"1px solid #7F1D1D"}}><button style={{...S.btn,width:"100%",background:"#7F1D1D",color:"#FCA5A5",fontSize:14,padding:10,borderRadius:6}} onClick={()=>{onUpdVendor(vn.id,{declined:true,declinedDate:new Date().toISOString().split("T")[0]});const others=(task.vendors||[]).filter((v2: any)=>v2.id!==vn.id);const allDeclined=others.every((v2: any)=>v2.declined);if(allDeclined){onUpdateTask({status:"declined"});}
...
<button style={{...S.sm,color:"#6B7280",marginLeft:8,fontSize:12}} onClick={()=>{const sc=task.status==="declined"?{status:"assigned"}:undefined;onUpdVendor(vn.id,{declined:false,declinedDate:""},sc);}}>Undo</button></div>}
</div>;})}
{!isOEM&&!isCR&&(()=>{const hasTasks=(task.workTasks||[]).length>0&&(task.workTasks||[]).every((w: any)=>w.desc&&w.desc.trim().length>0);const hasMedia=photos.length>0;const mediaSkipped=task.mediaSkipped===true;const mediaReady=hasMedia||mediaSkipped;const noVendorYet=(task.vendors||[]).length===0;
return <>
...
<button style={{...S.btn,flex:1,background:"#4C1D95",color:"#E9D5FF"}} onClick={()=>{const inp=document.createElement("input");inp.type="file";inp.multiple=true;(inp as any).onchange=hu;inp.click();}}>📁 Browse Files</button>
...
<div style={{fontSize:13,color:"#9CA3AF",marginTop:4}}>Vendor ETA: {(()=>{const sv2=(task.vendors||[]).find((x: any)=>x.bidLocked);return sv2?.etaDone?fmtDate(sv2.etaDone):"—";})()}</div>
</div>}
...
{(()=>{const sv2=(task.vendors||[]).find((x: any)=>x.selected);const vendorEta=sv2?.etaDone||"";return vendorEta?<div style={{fontSize:14,color:"#60A5FA",marginBottom:8}}>ETA Completion: <b>{fmtDate(vendorEta)}</b></div>:null;})()}
{isVendor?<button style={{...S.btn,background:"#16A34A",width:"100%",padding:12,fontSize:16}} onClick={()=>{const sv2=(task.vendors||[]).find((x: any)=>x.selected);onStart(sv2?.etaDone||new Date().toISOString().split("T")[0]);}}>🔧 Start Work</button>:<div style={{padding:10,background:"#0D0D1A",borderRadius:6,border:"1px dashed #166534",textAlign:"center",fontSize:13,color:"#6B7280"}}>⏳ Awaiting vendor to start work</div>}
</div>}
...
{task.etaComplete&&(()=>{let d=new Date(task.etaComplete);if(d.getFullYear()<100)d.setFullYear(d.getFullYear()+2000);return d<new Date()?<div style={{color:"#F87171",fontWeight:700,marginBottom:4}}>⚠️ PAST DUE — {Math.max(1,~~(((new Date() as any)-(d as any))/864e5))}d late</div>:null;})()}
...
{(()=>{const pts=(task.workTasks||[]).filter((w: any)=>w.isPart);const sv2=(task.vendors||[]).find((x: any)=>x.selected);const allPartsIn=pts.length===0||pts.every((w: any)=>{const li2=(sv2?.lineItems||[]).find((x: any)=>x.id===w.id)||{};return li2.declined||li2.partInstalled;});const noPhotosNeeded=cat.key==="mechanical"||cat.key==="oemdealer";const hasInspection=cat.key==="mechanical"&&(task.workTasks||[]).some((w: any)=>w.isInspection);const hasFindings=sv2&&(sv2.vendorFindings||[]).length>0;const noIssues=task.noIssuesFound||false;const mechBlock=hasInspection&&!hasFindings&&!noIssues;const activeTasks=(task.workTasks||[]).filter((w: any)=>{const li9=(sv2?.lineItems||[]).find((x: any)=>x.id===w.id)||{};return !li9.declined;});const taskCount=activeTasks.length;const roundStart=task?.roundPhotoStart||0;const newPhotos=photos.length-roundStart;const photosOk=noPhotosNeeded||(taskCount>0&&newPhotos>=taskCount)||taskCount===0;const canComplete=photosOk&&allPartsIn&&!mechBlock;
...
<label style={{display:"flex",alignItems:"center",gap:8,fontSize:15,color:noIssues?"#34D399":"#E5E7EB",fontWeight:600,cursor:"pointer"}}><input type="checkbox" style={{width:18,height:18}} checked={noIssues} onChange={(e: any)=>onUpdateTask({noIssuesFound:e.target.checked})}/> ✅ No Issues Found — Vehicle Passed Inspection</label>
</div>}
{isVendor?<button style={{...S.btn,background:canComplete?"#166534":"#4B5563",width:"100%",padding:10,cursor:canComplete?"pointer":"not-allowed",opacity:canComplete?1:0.5}} onClick={(e: any)=>{if(!canComplete)return;e.target.disabled=true;onCmp();setExp(false);}}>✅ Mark Complete</button>:<div style={{padding:10,background:"#0D0D1A",borderRadius:6,border:"1px dashed #166534",textAlign:"center",fontSize:13,color:"#6B7280"}}>⏳ Awaiting vendor to mark complete</div>}
...
{(()=>{const winnerVn=(task.vendors||[]).find((v2: any)=>v2.selected);if(!winnerVn)return null;const accLines=(winnerVn.lineItems||[]).filter((x: any)=>x.accepted&&!x.declined);const total=accLines.reduce((s: any,x: any)=>s+(Number(x.price)||0),0);const ws=accLines.filter((x: any)=>x.costType==="ws"||!x.costType).reduce((s: any,x: any)=>s+(Number(x.price)||0),0);const ret=accLines.filter((x: any)=>x.costType==="retail").reduce((s: any,x: any)=>s+(Number(x.price)||0),0);return <div style={{background:"#0D0D1A",borderRadius:6,padding:10,marginBottom:10}}>
{accLines.map((li: any,li_idx: any)=><div key={li_idx} style={{display:"flex",justifyContent:"space-between",padding:"4px 0",fontSize:12,color:"#E5E7EB",borderBottom:li_idx<accLines.length-1?"1px solid #1E1E32":"none"}}>
<span>{li.desc}</span>
...
<div style={{display:"flex",gap:6}}>
<button style={{...S.btn,flex:1,background:"#166534",color:"#FFF",padding:10,fontSize:14,fontWeight:700}} onClick={()=>{const winnerVn=(task.vendors||[]).find((v2: any)=>v2.selected);if(!winnerVn){notify&&notify("⚠️ No vendor found");return;}const accLines=(winnerVn.lineItems||[]).filter((x: any)=>x.accepted&&!x.declined);const lockedTotal=accLines.reduce((s: any,x: any)=>s+(Number(x.price)||0),0);const lockedWS=accLines.filter((x: any)=>x.costType==="ws"||!x.costType).reduce((s: any,x: any)=>s+(Number(x.price)||0),0);const lockedRetail=accLines.filter((x: any)=>x.costType==="retail").reduce((s: any,x: any)=>s+(Number(x.price)||0),0);onUpdateTask({approvedForPayment:true,approvedPaymentDate:new Date().toISOString().split("T")[0],approvedBy:currentUser?.first_name?currentUser.first_name+" "+(currentUser.last_name||""):"Buyer",lockedTotal:lockedTotal,lockedWS:lockedWS,lockedRetail:lockedRetail,lockedVendorName:winnerVn.name});if(typeof fireEmail==="function"&&vehicle){fireEmail("recon_approved_for_payment",{vehicle:vData(vehicle),buyer:vehicle.buyingBroker,vendor:{name:winnerVn.name,email:winnerVn.email},seller:vehicle.sellingBroker,category:cat.label,categoryKey:cat.key,lineItems:accLines.map((x: any)=>({desc:x.desc,price:x.price,costType:x.costType||"ws"})),lockedTotal,lockedWS,lockedRetail,approvedBy:currentUser?.first_name?currentUser.first_name+" "+(currentUser.last_name||""):"Buyer",approvedDate:new Date().toISOString().split("T")[0],location:vehicle?.location||"PHX"});}notify&&notify("✅ Approved for payment — AP team notified");}}>✅ Approve for Payment</button>
<button style={{...S.btn,background:"#7F1D1D",color:"#FCA5A5",padding:"10px 14px",fontSize:13,fontWeight:700}} onClick={()=>{const reason=prompt("Why are you disputing this work?\n\n(e.g. work incomplete, wrong color, photos don't show finished work)");if(!reason)return;const winnerVn=(task.vendors||[]).find((v2: any)=>v2.selected);onUpdateTask({disputed:true,disputedDate:new Date().toISOString().split("T")[0],disputedReason:reason,disputedBy:currentUser?.first_name?currentUser.first_name+" "+(currentUser.last_name||""):"Buyer",status:"started",dateCompleted:null});if(typeof fireEmail==="function"&&vehicle&&winnerVn){fireEmail("recon_disputed",{vehicle:vData(vehicle),buyer:vehicle.buyingBroker,vendor:{name:winnerVn.name,email:winnerVn.email},seller:vehicle.sellingBroker,category:cat.label,categoryKey:cat.key,reason:reason,disputedBy:currentUser?.first_name?currentUser.first_name+" "+(currentUser.last_name||""):"Buyer",disputedDate:new Date().toISOString().split("T")[0]});}notify&&notify("⚠️ Work disputed — vendor notified");}}>⚠️ Dispute</button>
</div>
</div>}
...
{task.dateStarted&&task.dateCompleted&&<span style={{fontSize:13,color:"#9CA3AF",marginLeft:8}}>({Math.max(1,~~(((new Date(task.dateCompleted) as any)-(new Date(task.dateStarted) as any))/864e5))}d to complete)</span>}</div>
