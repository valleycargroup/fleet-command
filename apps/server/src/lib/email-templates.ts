...
function vBlock(v: any): string {
  ...
  const stockNum = clean(v.stockNumber) || clean(v.stock_number) || '';
  const vinLabel = clean(v.fullVin) || clean(v.vin) || clean(v.vin8) || '—';
  const stockLine = stockNum ? `<span style="font-size:12px;font-weight:700;color:#818CF8;font-family:monospace;margin-left:6px">Stock #${stockNum}</span>` : '';
  return `<div style="padding:12px 16px;background:#0D0D1A;border-radius:8px;border:1px solid #2A2A3E;margin-bottom:20px"><b style="color:#FFF;font-size:16px">${v.year || ''} ${v.make || ''} ${v.model || ''} ${trim}</b>${stockLine}<br><span style="font-size:14px;color:#9CA3AF">VIN: ${vinLabel} • ${color} • ${(v.miles || 0).toLocaleString()} mi • ${location}</span>${soldTo ? `<br><span style="color:#34D399;font-weight:600">Sold to: ${soldTo}${soldDate ? ' — ' + fmtDate(soldDate) : ''}</span>` : ''}</div>`;
}
...
function dRow(v: any, rHtml: string, bc: string): string {
  const dStock = v.stockNumber || v.stock_number || '';
  const dVin = v.fullVin || v.vin || v.vin8 || '—';
  return `<table width="100%" cellpadding="0" cellspacing="0" style="background:#0D0D1A;border-radius:8px;margin-bottom:6px;border-left:3px solid ${bc||'#3B82F6'};"><tr><td style="padding:12px;"><a href="${vLink(v.id)}" style="text-decoration:none;"><b style="color:#FFF;font-size:13px;">${v.year} ${v.make} ${v.model}</b></a>${dStock ? `<span style="font-size:11px;font-weight:700;color:#818CF8;font-family:monospace;margin-left:6px">${dStock}</span>` : ''}<br><span style="font-size:11px;color:#6B7280;">VIN: ${dVin}${v.soldTo||v.sold_to?' • Sold to: '+(v.soldTo||v.sold_to):''}</span></td><td style="padding:12px;text-align:right;">${rHtml}</td></tr></table>`;
}
...
function greet(d: any): string {
  const role = d.recipientRole || 'other';
  const buyer  = clean(d.buyer)  || clean(d.vehicle?.buyingBroker);
  const seller = clean(d.seller) || clean(d.vehicle?.sellingBroker);
  return role === 'seller' ? (seller || buyer || 'Team') : (buyer || seller || 'Team');
}
  ...
  vendor_payment_pending_digest: (d) => {
    ...
    const jobRow = (j: any) => {
      ...
      const soldTag = j.soldTo
        ...
        : '';
      return `<table width="100%" cellpadding="0" cellspacing="0" style="background:#0D0D1A;border-radius:8px;border-left:3px solid #34D399;margin-bottom:6px;"><tr><td style="padding:12px;"><div style="margin-bottom:4px;">${link}${soldTag}</div><div style="font-size:11px;color:#6B7280;font-family:monospace;">VIN ${j.vin || j.vin8 || '—'} • ${j.color || ''} • ${(j.miles || 0).toLocaleString()} mi • ${j.location || ''}</div><div style="font-size:11px;color:#9CA3AF;margin-top:4px;">${j.categoryIcon} ${j.categoryLabel} — Approved ${j.approvedDate ? j.approvedDate : '—'} by ${j.approvedBy || 'Buyer'}</div></td><td style="padding:12px;text-align:right;white-space:nowrap;"><div style="font-size:16px;font-weight:700;color:#FBBF24;">$${(j.lockedTotal || 0).toLocaleString()}</div><div style="font-size:10px;color:#6B7280;margin-top:2px;">W/S $${(j.lockedWS||0).toLocaleString()} • RT $${(j.lockedRetail||0).toLocaleString()}</div></td></tr></table>`;
    };
